...
class IntegralOccupancyMap(object):
    def __init__(self, height, width, mask, eps1, eps2):
        self.height = height
        self.width = width
    ...
    def sample_position(self, size_x, size_y, random_state, pos):
        return query_integral_image(self.integral, size_x, size_y,
                                    random_state, pos, self.eps1, self.eps2)
...
class WordCloud(object):
    ...
    def __init__(self, font_path=None, width=400, height=200, margin=2,
                 ...
                 include_numbers=False, min_word_length=0, collocation_threshold=30, pos_dict=None,
                 max_word_length=20, alpha=0.3, eps1=30, eps2=1.):
    ...
    def generate_from_frequencies(self, frequencies, max_font_size=None):  # noqa: C901
        ...
        if len(word) <= self.max_word_length and word not in self.stopwords]
        ...
        occupancy = IntegralOccupancyMap(height, width, boolean_mask, self.eps1, self.eps2)
        ...
        for i, (word, freq) in enumerate(frequencies):
            print(i, word, end=' ')
            if freq == 0:# or word in self.stopwords:
                continue
            ...
            if rs != 0:
                #font_size = int(round((rs * (freq / float(last_freq))
                                       #+ (1 - rs)) * font_size))
                font_size = round(self.max_font_size * freq ** self.alpha)
            if True:#random_state.random() < self.prefer_horizontal:
                orientation = None
            else:
            ...
            while True:
                ...
                result = occupancy.sample_position(box_size[1] + self.margin,
                                                   ...
                                                   random_state, self.pos_dict[word])
            ...
            used_frequencies.append((word, freq))
        ...
        self.layout_ = list(zip(used_frequencies, font_sizes, positions,
                                orientations, colors))
        return self
